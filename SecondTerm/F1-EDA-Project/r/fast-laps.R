...
plot <- ggplot(data, aes(x = Compound, y = AvgLapTime, color = TemperatureCondition, shape = HumidityCondition)) +
  geom_point(size = 4) +
  facet_wrap(~EventName) +  # Crea facetas para cada Gran Premio
  theme_minimal() +
...
scale_color_manual(values = c("Hot" = "red", "Mild" = "orange", "Cold" = "blue"))

print(plot)

ggsave("./csvs/performance_weather_conditions_interaction_plot.png", plot, width = 12, height = 8, dpi = 300)
